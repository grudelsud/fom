...
--
-- Host: localhost
-- Generation Time: Feb 25, 2011 at 02:50 PM
...
CREATE TABLE IF NOT EXISTS `fom_link` (
  ...
  PRIMARY KEY (`id_link`),
  UNIQUE KEY `uri` (`uri`)
) ENGINE=InnoDB  DEFAULT CHARSET=latin1 AUTO_INCREMENT=47811 ;
...
) ENGINE=InnoDB  DEFAULT CHARSET=latin1 COMMENT='association between geo coords and user defined names' AUTO_INCREMENT=109154 ;
...
CREATE TABLE IF NOT EXISTS `fom_post` (
  ...
  KEY `src_id` (`src_id`)
) ENGINE=InnoDB  DEFAULT CHARSET=latin1 COMMENT='contains all sources indexed for search and clustering' AUTO_INCREMENT=559430 ;
...
CREATE TABLE IF NOT EXISTS `fom_postlink` (
  ...
  KEY `id_post` (`id_post`),
  KEY `id_link` (`id_link`)
) ENGINE=InnoDB  DEFAULT CHARSET=latin1 AUTO_INCREMENT=48836 ;
