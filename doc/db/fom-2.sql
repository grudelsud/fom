...
--
-- Host: localhost
-- Generation Time: Oct 24, 2010 at 06:44 PM
...
CREATE TABLE IF NOT EXISTS `fom_post` (
  ...
  `meta` varchar(255) default NULL COMMENT 'contains: src, stauts, tw_statusid, tw_replyto',
  PRIMARY KEY  (`id_post`),
...
CREATE TABLE IF NOT EXISTS `fom_term` (
  ...
  `uri` varchar(255) default NULL COMMENT 'contains reference to rdf uri if present, e.g. from dbpedia',
  PRIMARY KEY  (`id_term`),
...
CREATE TABLE IF NOT EXISTS `fom_vocabulary` (
  ...
  `description` varchar(45) default NULL,
  `owl` varchar(255) default NULL COMMENT 'contains reference to related owl if present',
